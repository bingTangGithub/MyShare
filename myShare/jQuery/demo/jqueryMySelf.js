...
function $(a,c) {
    ...
    // Prototype.js是jQuery出来前比较热的一个库
    // 这个是判断这个对象是否存在，存在就说明加载了这个库，这个库会用到$这个变量，和jQuery会冲突，要做向后的兼容处理
    ...
    var self = {
        cur: $.Select(a,c),  // 被选择的元素组成的数组
        
        // The only two getters
        size: function() {
            return this.get().length;  // 获取的元素组成数组的长度
        },
        // 可获取 dom 对象
        get: function(i) {
            if ( i == null ) {
                ...
                return (this.get())[i];  // i 有值时，获取数组的第 i 个
        },
        "_get": function(i) {
            if ( i == null )
                return this.cur;
            else
                return this.cur[i];  // 将$元素转化为原生标签元素
        },
        
        // 将 jq 对象转化为 dom 对象，(为了使用 dom 的方法)，操作获取的 dom 对象
        each: function(f) {
            // console.log('this::::::：：：：', this);
            for ( var i = 0; this.cur && i < this._get().length; i++ ) {
                if ( this._get(i) ) {
                    this._get(i).$$tmpFunc = f;
                    this._get(i).$$tmpFunc(i);  // 为了给 dom 对象设置方法，因为此时执行回调函数的对象为 this._get(i)
        ...
        set: function(a,b) {
            return this.each(function(){
                // console.log('set 中 this::::', this);  // 用each 包装后，这里的this即为 dom 对象
                
                if ( b == null ) {  // 只有一个a参数时，将a对象的 value 赋值给选中的dom对象的key
                    for ( var j in a ) {
                        this[j] = a[j];
                    }
        ...
        wrap: function() {
            // 因为传进来的参数是字符串，不能 cloneNode，所以，先通过 clean方法将其转化为dom 数组 
            var a = $.clean(arguments); 
            return this.each(function(){
                var b = a[0].cloneNode(true); //节点才能cloneNode
                this.parentNode.insertBefore( b, this );  // 在 this dom 对象之前插入新的  b 元素（插入 this dom 中，作为前一个大哥节点）
                console.log('appendChild前的b:::::', b);
                while ( b.firstChild ) b = b.firstChild;
                b.appendChild( this ); // 将该dom作为要包装的dom的最小的孩子， 隐藏前后前边的log输出不一致
            });
        },
        // 幼儿
        append: function() {
            var a = $.clean(arguments);  // 通过 $.clean(), 将参数转化为dom对象数组
            console.log('a:::::::', a);
            return this.each(function(){
                for ( var i in a )
        ...
        find: function(t) {
            ...
            this.each(function(){
                ...
                // console.log('$.Select(t,this)::::::::::::', $.Select(t,this));
                // console.log('ret::::::::::::', ret);
            });
            this.old = old;
            this.cur = ret;
...
// 同一个类型比如click的事件，放在一个handlers数组中,不同的事件类型放在不同的handlers中
// handler.$$guid 是累加的，代表第几个事件
function addEvent(element, type, handler) {
    // debugger;
    if (!handler.$$guid) handler.$$guid = addEvent.guid++; //给每个事件一个编号，删除时通过找到该编号
    ...
    console.log("handler.$$guid:::::", handler.$$guid);
    element["on" + type] = handleEvent;
   
};
addEvent.guid = 1;
// 删除已经添加的事件，第三个参数要传函数名 || 不传
function removeEvent(element, type, handler) {
    ...
    if (element.events) {
        if (type && element.events[type]) {
            if ( handler ) {
                // console.log('addEvent.guid::::::', addEvent.guid);
                console.log('delete 前 handler::::::', handler);
                console.log('delete 前 handler.$$guid::::::', [handler.$$guid]);
                delete element.events[type][handler.$$guid]; // handler.$$guid 是addEvent时添加的编号，所以要从已经添加的事件中找
                // delete element.events[type][addEvent.guid-1];
                // addEvent.guid -= 1;
            } else { //删除某个类型的事件
                for ( var i in element.events[type] )
                    removeEvent( element, type, element.events[type][i] );
            }
        } else { // 没有删除该事件类型时，将该dom对象的所有事件类型逐个删掉
...
// 该 dom对象【第一个参数】的兄弟 dom对象们，返回它们的兄弟数组,其中 prev、next 分别代表第一个第一个参数的前兄后弟
// n 用于判断 type 的第n个值是否为第一个参数
$.sibling = function(a,n,e) {
    debugger;
  var type = new Array();
  var tmp = a.parentNode.childNodes;
  for ( var i = 0; i < tmp.length; i++ ) {
    if ( tmp[i].nodeType == 1 ) // 节点类型： 元素节点，将所有的 dom 节点放在该数组中
      type.push( tmp[i] );
    if ( tmp[i] == a )   // 第一个参数与将该节点相同时，记下它在数组中的下标： type.n
      type.n = type.length - 1;  //这里的n指第一个dom 在type 中所占的下标
  }
...
// 获取 第一个参数 e： dom元素的 p： 属性
$.getCSS = function(e,p) {
    console.log('e:::::::', e);
    console.log('e.style.color:::::::', e.style[p]);
  if (e.style[p])  //  只是 style 属性中的属性们，通过 class 等方式获取的不算
    return e.style[p];
  else if (e.currentStyle) // currentStyle是IE浏览器的一个属性，返回的是CSS样式对象
    return e.currentStyle[p];
  else if (document.defaultView && document.defaultView.getComputedStyle) { 
  // getComputedStyle是一个可以获取当前元素所有最终使用的CSS属性值。返回的是一个CSS样式对象
    p = p.replace(/([A-Z])/g,"-$1"); // 将匹配到的大写字母前边加-  ($0,$1....$9是表示正则匹配的组)
    console.log('p:::', p); // coLoR => co-Lo-R
    p = p.toLowerCase();
    console.log('p:::', p);
...
//  只获取参数中的 dom 节点，即将参数还原为 dom 对象
// 将参数中的字符串作为新创建的 div 的 innerHTML，
// 再将新创建的 div 的 childNodes 的 push 到 r 数组 使用时，此时数组中的的对象为具备dom 属性的各个dom
// 使用时通过取下边即可获取 dom，这样可将 转化为 string 类型的dom 还原为 dom  
$.clean = function(a) {
    var r = new Array();
    for ( var i = 0; i < a.length; i++ ) {
        if ( a[i].constructor == String ) {
            ...
            for ( var j = 0; j < a[i].length; j++ )
                r.push( a[i][j] );
        } else { // 本身是 dom 对象，直接clone
            r.push( a[i] ); //.cloneNode(true) );
        }
    }
    return r;
};
// 处理a，针对 not 函数，改变 this.cur, 通过 f，将 this.cur 中，跟 t 一样的 去掉
$.grep = function(a,f,s) {
  var r = new Array();
    if ( a != null )
        for ( var i = 0; i < a.length; i++ )
...
// 合并，将a所有的与b中不重复的值一个个放进b数组中
// 若 a 中的某个值与b的某个值重复时，才c 设置为 false ，不再添加到b中
// 在b 中添加a中的b不含有的元素
$.merge = function(a,b) {
    // for ( var i = 0; i < a.length; i++ ) {
    //     var c = true;
